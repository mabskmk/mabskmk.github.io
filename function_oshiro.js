//MENU-SHIRO ..atualizado 07/2021
function menushiro() {
document.getElementById('principal').innerHTML="<table border=0 width=750><td align=center><h1 id=nomeprinc><b>Shiro Project:RE</b></h1><form><b id=999></b><b id=998></b><b id=997></b><b id=689></b><b id=688></b><b id=675></b><b id=674></b><b id=671></b><b id=650></b><b id=638></b><b id=617></b><b id=613></b><b id=607></b><b id=606></b><b id=593></b><b id=590></b><b id=582></b><b id=577></b><b id=576></b><b id=569></b><b id=568></b><b id=567></b><b id=562></b><b id=561></b><b id=553></b><b id=552></b><b id=551></b><b id=546></b><b id=545></b><b id=539></b><b id=538></b><b id=537></b><b id=531></b><b id=530></b><b id=529></b><b id=523></b><b id=517></b><b id=514></b><b id=509></b><b id=508></b><b id=506></b><b id=503></b><b id=502></b><b id=501></b><b id=500></b><b id=499></b><b id=496></b><b id=495></b><b id=490></b><b id=489></b><b id=483></b><b id=481></b><b id=476></b><b id=475></b><b id=474></b><b id=470></b><b id=468></b><b id=464></b><b id=461></b><b id=458></b><b id=456></b><b id=455></b><b id=452></b><b id=451></b><b id=450></b><b id=448></b><b id=447></b><b id=446></b><b id=445></b><b id=444></b><b id=442></b><b id=441></b><b id=440></b><b id=439></b><b id=436></b><b id=435></b><b id=434></b><b id=432></b><b id=429></b><b id=428></b><b id=424></b><b id=423></b><b id=422></b><b id=419></b><b id=418></b><b id=417></b><b id=416></b><b id=415></b><b id=413></b><b id=412></b><b id=411></b><b id=410></b><b id=409></b><b id=408></b><b id=407></b><b id=406></b><b id=402></b><b id=401></b><b id=397></b><b id=396></b><b id=395></b><b id=394></b><b id=391></b><b id=390></b><b id=384></b><b id=382></b><b id=381></b><b id=379></b><b id=376></b><b id=374></b><b id=373></b><b id=370></b><b id=369></b><b id=367></b><b id=365></b><b id=364></b><b id=363></b><b id=362></b><b id=358></b><b id=356></b><b id=354></b><b id=353></b><b id=352></b><b id=351></b><b id=349></b><b id=347></b><b id=346></b><b id=345></b><b id=344></b><b id=343></b><b id=342></b><b id=341></b><b id=338></b><b id=337></b><b id=336></b><b id=334></b><b id=330></b><b id=329></b><b id=328></b><b id=323></b><b id=322></b><b id=321></b><b id=319></b><b id=318></b><b id=317></b><b id=315></b><b id=312></b><b id=310></b><b id=307></b><b id=306></b><b id=301></b><b id=300></b><b id=299></b><b id=297></b><b id=296></b><b id=291></b><b id=290></b><b id=289></b><b id=288></b><b id=284></b><b id=282></b><b id=281></b><b id=280></b><b id=279></b><b id=275></b><b id=274></b><b id=270></b><b id=268></b><b id=264></b><b id=261></b><b id=260></b><b id=258></b><b id=257></b><b id=256></b><b id=255></b><b id=253></b><b id=250></b><b id=248></b><b id=244></b><b id=243></b><b id=240></b><b id=239></b><b id=238></b><b id=233></b><b id=231></b><b id=227></b><b id=225></b><b id=214></b><b id=212></b><b id=210></b><b id=208></b><b id=207></b><b id=205></b><b id=204></b><b id=200></b><b id=199></b><b id=197></b><b id=195></b><b id=194></b><b id=193></b><b id=190></b><b id=189></b><b id=188></b><b id=185></b><b id=184></b><b id=182></b><b id=180></b><b id=178></b><b id=177></b><b id=175></b><b id=174></b><b id=173></b><b id=172></b><b id=169></b><b id=168></b><b id=167></b><b id=165></b><b id=164></b><b id=163></b><b id=161></b><b id=158></b><b id=157></b><b id=155></b><b id=154></b><b id=150></b><b id=149></b><b id=148></b><b id=147></b><b id=146></b><b id=145></b><b id=144></b><b id=142></b><b id=141></b><b id=139></b><b id=137></b><b id=136></b><b id=133></b><b id=132></b><b id=131></b><b id=130></b><b id=128></b><b id=127></b><b id=126></b><b id=125></b><b id=124></b><b id=123></b><b id=121></b><b id=118></b><b id=117></b><b id=116></b><b id=115></b><b id=114></b><b id=113></b><b id=112></b><b id=109></b><b id=108></b><b id=107></b><b id=106></b><b id=104></b><b id=103></b><b id=102></b><b id=101></b><b id=100></b><b id=99></b><b id=98></b><b id=95></b><b id=93></b><b id=91></b><b id=89></b><b id=86></b><b id=85></b><b id=84></b><b id=83></b><b id=82></b><b id=81></b><b id=78></b><b id=77></b><b id=76></b><b id=74></b><b id=72></b><b id=71></b><b id=70></b><b id=67></b><b id=66></b><b id=64></b><b id=63></b><b id=62></b><b id=61></b><b id=60></b><b id=59></b><b id=58></b><b id=57></b><b id=55></b><b id=54></b><b id=52></b><b id=51></b><b id=50></b><b id=47></b><b id=44></b><b id=43></b><b id=42></b><b id=41></b><b id=40></b><b id=39></b><b id=37></b><b id=36></b><b id=35></b><b id=33></b><b id=32></b><b id=31></b><b id=29></b><b id=28></b><b id=25></b><b id=23></b><b id=22></b><b id=21></b><b id=18></b><b id=17></b><b id=11></b><b id=10></b><b id=9></b><b id=7></b><b id=5></b><b id=3></b><b id=2></b><br><br><input type=button onClick='ppmain(1)' value='.Clean.'></form><br><tr></table>"
btnnew.disabled=false;
btnchar.disabled=false;
btnstg.disabled=false;
btnfullg.disabled=false;
btntuto.disabled=false;
btnyotu.disabled=false;
for (x=2;x<1000;x++) {
oshiro((x),1)
}
oshiro(689,2);
}

function vioshiro() {
rand=Math.floor(Math.random() * 999);
//rand=Math.floor((Math.random() * 10) + 999);
oshiro(rand,3);
if (special!=''){
document.getElementById('videoshowcase').innerHTML="<iframe src=https://www.youtube.com/embed/"+special+"?autoplay=1&mute=1 width='500' height='300' frameborder='0'></iframe><br>CASTLE ("+rand+"): <b><font color=blue>"+mydiv+"</font></b>"
}
else {
vioshiro();//while
}
tempo=setTimeout('vichar()',50000);
}

//CASTLES
function oshiro(numero,cod) {
mydiv='';special='';destroyed='';
switch (numero) {
case 2:
mydiv='Matsumae_Castle',special='',destroyed=''
break;
case 3:
mydiv='Ne_Castle',special='3-Y_w-S1Y7w',destroyed=''
break;
case 5:
mydiv='Morioka_Castle',special='',destroyed=''
break;
case 7:
mydiv='Sendai_Castle',special='GPdtMfm5WDQ',destroyed=''
break;
case 9:
mydiv='Kurokawa_Castle',special='',destroyed=''
break;
case 10:
mydiv='Aizuwakamatsu_Castle',special='',destroyed=''
break;
case 11:
mydiv='Yamagata_Castle',special='Fi0M_9tp_fw',destroyed='8nTPwvEUoM0'
break;
case 17:
mydiv='Baba_Castle',special='',destroyed=''
break;
case 18:
mydiv='Mito_Castle',special='',destroyed=''
break;
case 21:
mydiv='Maebashi_Castle',special='',destroyed=''
break;
case 22:
mydiv='Utsunomiya_Castle',special='n0vb1dlCKn4',destroyed=''
break;
case 23:
mydiv='Tobiyama_Castle',special='uAHi_Ztgcdg',destroyed='9BM1ADyO_9s'
break;
case 25:
mydiv='Otaki_Castle',special='',destroyed=''
break;
case 28:
mydiv='Takiyama_Castle',special='',destroyed=''
break;
case 29:
mydiv='Odawara_Castle',special='jNQQhHBcNuU',destroyed=''
break;
case 31:
mydiv='Shinpu_Castle',special='w2VZzw8jvec',destroyed=''
break;
case 32:
mydiv='Nabe-Lid_Castle',special='',destroyed=''
break;
case 33:
mydiv='Komoro_Castle',special='tq_0fNyDBAY',destroyed=''
break;
case 35:
mydiv='Sunpu_Castle',special='lmvqUzERGWg',destroyed=''
break;
case 36:
mydiv='Komakiyama_Castle',special='jaSjcTdXpzc',destroyed=''
break;
case 37:
mydiv='Yanaginomaru',special='',destroyed=''
break;
case 39:
mydiv='Nagoya_Castle',special='1rmBMOSJRs0',destroyed=''
break;
case 40:
mydiv='Inabayama_Castle',special='',destroyed=''
break;
case 41:
mydiv='Gifu_Castle',special='K-UxTpHJTpk',destroyed=''
break;
case 42:
mydiv='Naegi_Castle',special='_qhqZKwZoA4',destroyed=''
break;
case 43:
mydiv='Sakado_Castle',special='',destroyed=''
break;
case 44:
mydiv='Toyama_Castle',special='',destroyed=''
break;
case 47:
mydiv='Kitanosho_Castle',special='',destroyed=''
break;
case 50:
mydiv='Kanegasaki_Castle',special='',destroyed=''
break;
case 51:
mydiv='Sakamoto_Castle',special='C92FtgcEVJU',destroyed=''
break;
case 52:
mydiv='Sawayama_Castle',special='OFTZXMMAOXY',destroyed=''
break;
case 54:
mydiv='Niki_Museum',special='',destroyed=''
break;
case 55:
mydiv='Iga_Ueno_Castle',special='wvT8FX3SQkA',destroyed=''
break;
case 57:
mydiv='Tamonyama_Castle',special='',destroyed=''
break;
case 58:
mydiv='Ryuozan_Castle',special='',destroyed=''
break;
case 59:
mydiv='Ishiyama_Gobo',special='eZpvTpmdoWk',destroyed=''
break;
case 60:
mydiv='Osaka_Castle',special='OD3-GDk09dw',destroyed=''
break;
case 61:
mydiv='Iimoriyama_Castle',special='mnXy1KQct20',destroyed=''
break;
case 62:
mydiv='Oniga_Castle',special='Z_YdR2qX4Lk',destroyed=''
break;
case 63:
mydiv='Saiga_Castle',special='',destroyed=''
break;
case 64:
mydiv='Miki_Castle',special='',destroyed=''
break;
case 66:
mydiv='Okayama_Castle',special='5b2qJoWXh04',destroyed=''
break;
case 67:
mydiv='Takamatsu_Castle',special='',destroyed=''
break;
case 70:
mydiv='Shikano_Castle',special='',destroyed=''
break;
case 71:
mydiv='Tottori_Castle',special='',destroyed=''
break;
case 72:
mydiv='Tsuzura_Ojo',special='',destroyed=''
break;
case 74:
mydiv='Tsuwano_Castle',special='vtFh4_Cs0Vw',destroyed=''
break;
case 76:
mydiv='Takamatsu_Castle',special='8Dhv8hLl4n8',destroyed=''
break;
case 77:
mydiv='Hiketa_Castle',special='',destroyed=''
break;
case 78:
mydiv='Shozui_Castle',special='s-KIHhUz_N4',destroyed=''
break;
case 81:
mydiv='Kochi_Castle',special='',destroyed=''
break;
case 82:
mydiv='Tsurusaki_Castle',special='',destroyed=''
break;
case 83:
mydiv='Fukuoka_Castle',special='',destroyed=''
break;
case 84:
mydiv='Wakamatsu_Castle',special='X4v67xonjqc',destroyed=''
break;
case 85:
mydiv='Shimabara_Castle',special='vCeog9LL53Y',destroyed=''
break;
case 86:
mydiv='Muranaka Castle',special='CRM_7exlx_4',destroyed='KP5y6TxG14o'
break;
case 89:
mydiv='Hirado_Castle',special='',destroyed=''
break;
case 91:
mydiv='Uto_Castle',special='',destroyed=''
break;
case 93:
mydiv='Kagoshima_Castle',special='mRl1teRPzA4',destroyed=''
break;
case 95:
mydiv='Urasoe_Castle',special='',destroyed=''
break;
case 98:
mydiv='Noshima_Castle',special='',destroyed=''
break;
case 99:
mydiv='Gassantoda_Castle',special='',destroyed=''
break;
case 100:
mydiv='Wakimoto_Castle',special='rbm84TEtR0M',destroyed=''
break;
case 101:
mydiv='Ichijodani_Castle',special='',destroyed=''
break;
case 102:
mydiv='Sunomata_Castle',special='JSQFnsONqeQ',destroyed=''
break;
case 103:
mydiv='Iwabitsu_Castle',special='',destroyed=''
break;
case 104:
mydiv='Fukuyama_Castle',special='FVMV6s38lgU',destroyed=''
break;
case 106:
mydiv='Harajo',special='',destroyed=''
break;
case 107:
mydiv='Negoro_Castle',special='',destroyed=''
break;
case 108:
mydiv=' Toba_Castle',special='',destroyed=''
break;
case 109:
mydiv='Kasugayama_Castle',special='yWnKSXE4p24',destroyed=''
break;
case 112:
mydiv='Imahama_Castle',special='',destroyed=''
break;
case 113:
mydiv='Nagahama Castle',special='ndQ_o4-21ZE',destroyed='dK2u9sz1MEs'
break;
case 114:
mydiv='Hachioji_Castle',special='',destroyed=''
break;
case 115:
mydiv='Tagajo',special='',destroyed=''
break;
case 116:
mydiv='Matsukura_Castle',special='',destroyed=''
break;
case 117:
mydiv='Frankenstein_Castle',special='',destroyed=''
break;
case 118:
mydiv='Yoita_Castle',special='',destroyed=''
break;
case 121:
mydiv='Hikone_Castle',special='HeYEx8C1JXE',destroyed=''
break;
case 123:
mydiv='Yanagawa_Castle',special='vA2841iDPkc',destroyed=''
break;
case 124:
mydiv='Ishigakiyama_Castle',special='rjUPwSet-JI',destroyed=''
break;
case 125:
mydiv='Imabari_Castle',special='i-1YIe-bVe8',destroyed=''
break;
case 126:
mydiv='Sanada_Maru',special='',destroyed=''
break;
case 127:
mydiv='Kannonji_Castle',special='hGwoyGwUQpk',destroyed=''
break;
case 128:
mydiv='Bran_Castle',special='QlSLssyMobk',destroyed=''
break;
case 130:
mydiv='Oko_Castle',special='',destroyed=''
break;
case 131:
mydiv='Oyama_Gobo',special='',destroyed=''
break;
case 132:
mydiv='Sakuma_Kanazawa_Castle',special='',destroyed=''
break;
case 133:
mydiv='Bodhiyama_Castle',special='',destroyed='cifjmbBq2c0'
break;
case 136:
mydiv='Fukashi_Castle',special='wKMxYPP25Ys',destroyed=''
break;
case 137:
mydiv='Matsumoto_Castle',special='',destroyed=''
break;
case 139:
mydiv='Oyama_Castle',special='',destroyed=''
break;
case 141:
mydiv='Nakamurakan',special='',destroyed=''
break;
case 142:
mydiv='Soma_Nakamura_Castle',special='',destroyed=''
break;
case 144:
mydiv='Shiryokaku',special='m9sHUfdIHQI',destroyed=''
break;
case 145:
mydiv='Goryokaku',special='UoTSSYIAvzE',destroyed=''
break;
case 146:
mydiv='Maeda_Kanazawa_Castle',special='8c_npcIrg-4',destroyed=''
break;
case 147:
mydiv='Anping_Old_Fort',special='smqO17RvxRY',destroyed=''
break;
case 148:
mydiv='Orange_Castle',special='tCgA26Qum4M',destroyed=''
break;
case 149:
mydiv='Fort_Zeelandia',special='7_v9zv1vaw4',destroyed=''
break;
case 150:
mydiv='Shigisan_Castle',special='',destroyed=''
break;
case 154:
mydiv='Takaoka_Castle',special='CqND54k23-0',destroyed=''
break;
case 155:
mydiv='Yoshida-Koriyama_Castle',special='AOgJuD8D8uk',destroyed=''
break;
case 157:
mydiv='Kaerikumo_Castle',special='',destroyed=''
break;
case 158:
mydiv='Neuschwanstein_Castle',special='KszNez-80p0',destroyed='H1MbVZHXS-M'
break;
case 161:
mydiv='Fukushima_Castle',special='2Dt9Ru7UJe0',destroyed=''
break;
case 163:
mydiv='Kiyosu_Castle',special='4vs5bvwsOMk',destroyed=''
break;
case 164:
mydiv='Itami_Castle',special='',destroyed=''
break;
case 165:
mydiv='Arioka_Castle',special='MMR-sh6FcmY',destroyed=''
break;
case 167:
mydiv='Castel_Sant_Angelo',special='AQbebo3v9-A',destroyed=''
break;
case 168:
mydiv='Bitchu_Matsuyama',special='4GS0IHhp78c',destroyed=''
break;
case 169:
mydiv=' Itashima_Marukushi',special='',destroyed=''
break;
case 172:
mydiv='Jurakudai',special='CE6dttTDGk0',destroyed=''
break;
case 173:
mydiv='Oka_Castle',special='MZbNdIx1q54',destroyed=''
break;
case 174:
mydiv='Nakatsu_Castle',special='bE857S2ifpQ',destroyed=''
break;
case 175:
mydiv='Maruyama_Castle',special='',destroyed=''
break;
case 177:
mydiv='Chateau_Gaillard',special='YCRqbi6_dWE',destroyed=''
break;
case 178:
mydiv='Kamei_Castle',special='',destroyed=''
break;
case 180:
mydiv='Yukuepirachashi',special='h8OcDM4UPGA',destroyed='c08-WUsEUJA'
break;
case 182:
mydiv='Tokushima_Castle',special='',destroyed=''
break;
case 184:
mydiv='Kumamoto_Castle',special='H5kbg_Tm2EE',destroyed=''
break;
case 185:
mydiv='Kumamoto_Castle',special='',destroyed='Q9Qjt67Q830'
break;
case 188:
mydiv='Conwy_Castle',special='',destroyed=''
break;
case 189:
mydiv='Caernarfon_Castle',special='U1E7U6nxjC8',destroyed=''
break;
case 190:
mydiv='Yodo_Castle',special='',destroyed=''
break;
case 193:
mydiv='Iinoya_Castle',special='',destroyed=''
break;
case 194:
mydiv='Usuki_Castle',special='uH-Gc9t9QpM',destroyed=''
break;
case 195:
mydiv='Tsuyama_Castle',special='jfLPTMp-zrM',destroyed=''
break;
case 197:
mydiv='Yamatokoriyama_Castle',special='',destroyed=''
break;
case 199:
mydiv='Murao_Castle',special='',destroyed=''
break;
case 200:
mydiv='Hamada_Castle',special='gU6AU6A6Rf4',destroyed='DBg1BoKqlEg'
break;
case 204:
mydiv='Tanba_Kameyama',special='J4RSMqOUTKY',destroyed=''
break;
case 205:
mydiv='Iwadenyama_Castle',special='zdVwzxpfo8E',destroyed=''
break;
case 207:
mydiv='Yamanaka_Castle',special='E4A69CN6zBg',destroyed=''
break;
case 208:
mydiv='Hikima_Castle',special='0NI4fFpSDvM',destroyed=''
break;
case 210:
mydiv='Shirakawa-Komine_Castle',special='sFi6WIg8BAU',destroyed='ecod2Fzt07o'
break;
case 212:
mydiv='Iwakuni_Castle',special='',destroyed='GkTASN4SW5o'
break;
case 214:
mydiv='Forbidden_City',special='Q9xIfD1DK5g',destroyed='7CPbFCP753Q'
break;
case 225:
mydiv='Nagoya_Castle',special='',destroyed=''
break;
case 227:
mydiv='Nitta_Kanayama',special='W8kEjzdeo4U',destroyed=''
break;
case 231:
mydiv='Aoba_Castle',special='',destroyed=''
break;
case 233:
mydiv='Chinon_Castle',special='4jTZnFgf1VA',destroyed='w5mSTLn6XY0'
break;
case 238:
mydiv='Luxembourg',special='0MQww4f_VYM',destroyed=''
break;
case 239:
mydiv='Akashi_Castle',special='',destroyed='LempRNzyIqo'
break;
case 240:
mydiv='Yanagi_no_Gosho',special='',destroyed=''
break;
case 243:
mydiv='Pingyao_Ancient_City',special='',destroyed=''
break;
case 244:
mydiv='Schonbrunn_Palace',special='luRHnULqdTE',destroyed=''
break;
case 248:
mydiv='Kokura_Castle',special='',destroyed=''
break;
case 250:
mydiv='Tsujo',special='',destroyed=''
break;
case 253:
mydiv='Shingu_Castle',special='RHpQ-YaWals',destroyed=''
break;
case 255:
mydiv='Chambord_Castle',special='_dVv_vk_OHs',destroyed=''
break;
case 256:
mydiv='Warwick_Castle',special='',destroyed=''
break;
case 257:
mydiv='Edo_clan',special='',destroyed=''
break;
case 258:
mydiv='Minato_Castle',special='Ks_wdV5a6ZQ',destroyed=''
break;
case 260:
mydiv='Shigisan_Castle',special='riHE4E87stM',destroyed='zdIN2rat-_I'
break;
case 261:
mydiv='Chateau_de_Fontainebleau',special='mYAcbsVX08U',destroyed=''
break;
case 264:
mydiv='Kinojo',special='0wOr4_bqnp8',destroyed=''
break;
case 268:
mydiv='Luoyang_Castle',special='8mEBM2guqpo',destroyed=''
break;
case 270:
mydiv='Tendo_Castle',special='',destroyed=''
break;
case 274:
mydiv='Kamakura_Castle',special='',destroyed=''
break;
case 275:
mydiv='Shinagawa_Daiba',special='',destroyed=''
break;
case 279:
mydiv='Kingame Castle',special='9lXsvoIC_Us',destroyed='AKu2YMfQIg8'
break;
case 280:
mydiv='Handan',special='',destroyed=''
break;
case 281:
mydiv='Shoryuji_Castle',special='wKjUM2H6mCY',destroyed=''
break;
case 282:
mydiv='Nesvizh_Castle',special='D4I8dGl9J6M',destroyed=''
break;
case 284:
mydiv='Spis_Castle',special='',destroyed=''
break;
case 288:
mydiv='Gujo_Hachiman',special='',destroyed=''
break;
case 289:
mydiv='Omihachiman_Castle',special='',destroyed=''
break;
case 290:
mydiv='Hyogo_Castle',special='sPVifQ3svqk',destroyed=''
break;
case 291:
mydiv='Jinhua_Mountain_Castle',special='',destroyed=''
break;
case 296:
mydiv='Sogo_Castle',special='ZhAnAhZpNSk',destroyed=''
break;
case 297:
mydiv='Takasaki_Castle',special='',destroyed=''
break;
case 299:
mydiv='Hachigata_Castle',special='8b7MkmPq75o',destroyed=''
break;
case 300:
mydiv='Nihonmatsu_Castle',special='W5rabxH4SvU',destroyed=''
break;
case 301:
mydiv='Hasedo_Castle',special='',destroyed=''
break;
case 306:
mydiv='Furukawa_Castle',special='',destroyed=''
break;
case 307:
mydiv='Toledo_Fortress',special='fEq5J9Hu2oo',destroyed='yRXvRxLbK80'
break;
case 310:
mydiv='Sadowara_Castle',special='',destroyed=''
case 312:
mydiv='Rocca_Maggiore',special='v-S9LqQ5a7g',destroyed=''
break;
case 315:
mydiv='Lijiang_Ancient_City',special='QwJHpg_PRKo',destroyed=''
break;
case 317:
mydiv='Hitoyoshi_Castle',special='',destroyed=''
break;
case 318:
mydiv='Kamiakasaka_Castle',special='vVd74pWhWmY',destroyed=''
break;
case 319:
mydiv='Amagasaki_Castle',special='',destroyed=''
break;
case 321:
mydiv='Zeze_Castle',special='nPJj8DlB-do',destroyed=''
break;
case 322:
mydiv='Matsusaka_Castle',special='M80o5erRXXg',destroyed=''
break;
case 323:
mydiv='Ryukyu_Castle',special='',destroyed=''
break;
case 328:
mydiv='Himeji Castle',special='-aH-wkD-aH8',destroyed='ijPTN_6RM5c'
break;
case 329:
mydiv='Nakijin_Castle',special='',destroyed=''
break;
case 330:
mydiv='Ryugu_Castle',special='v9bYLh_687g',destroyed='ppG2QiQPOJw'
break;
case 334:
mydiv='Momochi Fortress',special='7xBpNHd0EQM',destroyed=''
break;
case 336:
mydiv='Kuno_Castle',special='0jUyhVVEX74',destroyed=''
break;
case 337:
mydiv='Akutagawa_Hill_Castle',special='',destroyed=''
break;
case 338:
mydiv='Tachibanayama_Castle',special='OjW98X1GqPw',destroyed=''
break;
case 341:
mydiv='Edo_Castle',special='Rde56huiZVk',destroyed=''
break;
case 342:
mydiv='Kindai_Castle',special='068TJQPm2qw',destroyed=''
break;
case 343:
mydiv='Chiyoda_Castle',special='dJqtsB8Zib8',destroyed=''
break;
case 344:
mydiv='Amazaki_Castle',special='xY2NCa4LPXY',destroyed=''
break;
case 345:
mydiv='Tora_Wojo',special='',destroyed=''
break;
case 346:
mydiv='Muromachi_No',special='',destroyed=''
break;
case 347:
mydiv='Hohenzollern_Castle',special='pozvLYbe37A',destroyed=''
break;
case 349:
mydiv='Komine_Castle',special='-SIWjTMLvTQ',destroyed=''
break;
case 351:
mydiv='Tatsuoka_Castle',special='WhAXQPQVodQ',destroyed=''
break;
case 352:
mydiv='Cochem_Rihisbruk_Castle',special='',destroyed=''
break;
case 353:
mydiv='Xuchang_Castle',special='qAkna2sJM0U',destroyed='lfXzzQ4qxgs'
break;
case 354:
mydiv='Okazaki_Castle',special='5UunQENrlIU',destroyed=''
break;
case 356:
mydiv='Chihaya_Castle',special='PlkOU2B5H0U',destroyed=''
break;
case 358:
mydiv='Nobeoka_Castle',special='UOU6GK-4NpI',destroyed=''
break;
case 362:
mydiv='???',special='',destroyed=''
break;
case 363:
mydiv='Dazaifu',special='uNuOfhLiJFg',destroyed=''
break;
case 364:
mydiv='Uwajima_Castle',special='XBHogThJFWA',destroyed='vdXBswlnyss'
break;
case 365:
mydiv='Kurumi_Tokisaki',special='BK8CpTG2OyQ',destroyed='JtSq_qfP5Ew'
break;
case 367:
mydiv='Yamazaki_Castle',special='6nJdY_3EjS4',destroyed='4jhxsNPvT7Q'
break;
case 369:
mydiv='Fukiage_Castle',special='GaRIDR_3qMs',destroyed=''
break;
case 370:
mydiv='Urasoe Castle',special='zO7KysbjaEQ',destroyed=''
break;
case 373:
mydiv='Toyama_Castle',special='',destroyed=''
break;
case 374:
mydiv='Kaerikumo_Castle',special='',destroyed=''
break;
case 376:
mydiv='Maruoka_Castle',special='VUn1Rs_xR0s',destroyed='o45iIrKYWBM'
break;
case 379:
mydiv='Heiankyo',special='x2QMAe905Yw',destroyed='foRbkgxAl-I'
break;
case 381:
mydiv='Heidelberg_Castle',special='V9wJy3-3v8k',destroyed=''
break;
case 382:
mydiv='San_Leo_Castle',special='o3_cUdY9Ehs',destroyed=''
break;
case 384:
mydiv='Heijo-kyo',special='T-iNmbhWVso',destroyed='VGoX8eN-1Mo'
break;
case 390:
mydiv='Frankenstein_Castle',special='',destroyed=''
break;
case 391:
mydiv='Leuvenburg_Castle',special='',destroyed=''
break;
case 394:
mydiv='Hirai_Castle',special='JRYXOMihsPQ',destroyed=''
break;
case 395:
mydiv='Dover_Castle',special='',destroyed=''
break;
case 396:
mydiv='Conciergerie',special='yJC2yoyx8o0',destroyed='pxEq5c5fxlk'
break;
case 397:
mydiv='Ogijo',special='',destroyed=''
break;
case 401:
mydiv='Kishiwada_Castle',special='MylESsLgaJk',destroyed=''
break;
case 402:
mydiv='Estense_Castle',special='',destroyed=''
break;
case 406:
mydiv='Niitakayama_Castle',special='w4fsDJnP1Ik',destroyed=''
break;
case 407:
mydiv='Okishio_Castle',special='',destroyed=''
break;
case 408:
mydiv='Ako_Castle',special='',destroyed=''
break;
case 409:
mydiv='Yuki_Castle',special='',destroyed=''
break;
case 410:
mydiv='Karatsu_Castle',special='',destroyed=''
break;
case 411:
mydiv='Toompea_Castle',special='zPsEEohys6E',destroyed='ZiuIvnYSm3w'
break;
case 412:
mydiv='Mimasaka_Ichinose_Castle',special='',destroyed=''
break;
case 413:
mydiv='Chigachishi_Castle',special='YUFQIhQGARg',destroyed=''
break;
case 415:
mydiv='Camellia_Castle',special='93EuzXPjUtY',destroyed=''
break;
case 416:
mydiv='Tage_Castle',special='qHrWt-UWjh8',destroyed=''
break;
case 417:
mydiv='Sumoto_Castle',special='',destroyed=''
break;
case 418:
mydiv='Gura_Castle',special='',destroyed=''
break;
case 419:
mydiv='Sakura_Castle',special='',destroyed=''
break;
case 422:
mydiv='Kannabe_Castle',special='',destroyed=''
break;
case 423:
mydiv='Goryokaku',special='CF1oHsWYDmg',destroyed=''
break;
case 424:
mydiv='Utsunomiya_Castle',special='IinU9CHaY40',destroyed=''
break;
case 428:
mydiv='Miyako-gun_Castle',special='',destroyed=''
break;
case 429:
mydiv='Tokutan_Castle',special='',destroyed=''
break;
case 432:
mydiv='Eljudnir',special='Vcuo9XhDoc8',destroyed='KWAtr8U-5Ps'
break;
case 434:
mydiv='White_Emperor_Castle',special='',destroyed=''
break;
case 435:
mydiv='Uozu_Castle',special='uE0U9y2ze98',destroyed=''
break;
case 436:
mydiv='Ozu_Castle',special='',destroyed=''
break;
case 439:
mydiv='Kokura_Castle',special='',destroyed=''
break;
case 440:
mydiv='Takashima_Castle',special='2A4rwLGKmzM',destroyed=''
break;
case 441:
mydiv='Shiwa_Castle',special='qclSXd4k3Z0',destroyed=''
break;
case 442:
mydiv='Athens_Acropolis',special='',destroyed='gYwIft5mr2k'
break;
case 444:
mydiv='Minowa_Castle',special='',destroyed=''
break;
case 445:
mydiv='Ashikaga_school',special='iGAKm9W1B5M',destroyed=''
break;
case 446:
mydiv='Tower_of_Babel',special='mkxA4-VafaI',destroyed='jK9dGAXuxd8'
break;
case 447:
mydiv='Shobata_Castle',special='w_wcShLiA_Q',destroyed=''
break;
case 448:
mydiv='Yogaiyama_Castle',special='iUJYQR0Ll2Y',destroyed=''
break;
case 450:
mydiv='Sunomata_Ichiya_Castle',special='JweSnLo2H4M',destroyed=''
break;
case 451:
mydiv='Karasawayama_Castle',special='',destroyed=''
break;
case 452:
mydiv='Wakasa_Oniga_Castle',special='5JYsV2GZPDA',destroyed=''
break;
case 455:
mydiv='Gassan Castle',special='ap_iVplTe3Q',destroyed='dvjD0fy1E5M'
break;
case 456:
mydiv='Izu_Shimoda_Castle',special='M-2fpYF3b20',destroyed=''
break;
case 458:
mydiv='Ejiri_Castle',special='OOmTf3p-I9M',destroyed=''
break;
case 461:
mydiv='Rasnov_Fortress',special='GgBzdeTukFg',destroyed=''
break;
case 464:
mydiv='Yuzuki_Castle',special='',destroyed=''
break;
case 468:
mydiv='Lindisfarne_Castle',special='5dyDhWofwhI',destroyed=''
break;
case 470:
mydiv='Yamagata_Castle',special='reSlYRzPxOo',destroyed=''
break;
case 474:
mydiv='Eilean_Donan_Castle',special='k7DfNCj-lgY',destroyed=''
break;
case 475:
mydiv='Shuri_Castle',special='',destroyed=''
break;
case 476:
mydiv='Amazaki_Castle',special='R1oFqHwH4Pk',destroyed=''
break;
case 481:
mydiv='Yanagawa_Castle',special='FOWmcrupmsc',destroyed=''
break;
case 483:
mydiv='Tsuruga_Castle',special='p6cE3p4NoJw',destroyed=''
break;
case 489:
mydiv='Najima_Castle',special='',destroyed=''
break;
case 490:
mydiv='Miki_Castle',special='oi8XxrXCLsY',destroyed=''
break;
case 495:
mydiv='Omi_Mochizuki_Castle',special='',destroyed=''
break;
case 496:
mydiv='Pfalz_Castle',special='',destroyed=''
break;
case 499:
mydiv='Otsu_Castle',special='',destroyed=''
break;
case 500:
mydiv='Kajo',special='wvlWsd1ClgY',destroyed=''
break;
case 501:
mydiv='Kuwana_Castle',special='',destroyed=''
break;
case 502:
mydiv='Bichu_Takamatsu',special='',destroyed=''
break;
case 503:
mydiv='City_of_the_Moon',special='egJiiSX7n9s',destroyed='QhjUxW7OEV0'
break;

case 506:
mydiv='Winter_Palace',special='GO9FVbtjk2c',destroyed=''
break;
case 508:
mydiv='Iwatsurugi_Castle',special='4jW9i7zYM1E',destroyed=''
break;
case 509:
mydiv='Pingyao_Ancient_City',special='BEjA6Ij0BgY',destroyed=''
break;
case 514:
mydiv='Chantilly_Castle',special='',destroyed=''
break;
case 517:
mydiv='Kotor_Fortress',special='N2Uj_3vSvVM',destroyed='vL_y0wLjHK4'
break;
case 523:
mydiv='Shiryokaku',special='vKHkH7tYjRk',destroyed=''
break;
case 529:
mydiv='Odani_Castle',special='',destroyed='pEh8-UlNPls'
break;
case 530:
mydiv='Mrs_Orange',special='',destroyed=''
break;
case 531:
mydiv='Fukuyama_Castle',special='',destroyed=''
break;
case 537:
mydiv='Nanae_Daiba',special='WWSdFJvxSTk',destroyed=''
break;
case 538:
mydiv='Iinuma_Castle',special='DMhj1Kn-ZAQ',destroyed=''
break;
case 539:
mydiv='Onigashima',special='jKMKyygwLaU',destroyed=''
break;
case 545:
mydiv='Komaruyama_Castle',special='GaFyt6mOlqQ',destroyed=''
break;
case 546:
mydiv='Moorish_Castle',special='',destroyed=''
break;
case 551:
mydiv='Genna_Edo_Castle',special='6qKpRBIHE2A',destroyed=''
break;
case 552:
mydiv='Kanei_Edo_Castle',special='ZHX9uVKf3PU',destroyed='RY4HuOIUBck'
break;
case 553:
mydiv='Shirasagi_Castle',special='cZi82HdQ3ug',destroyed=''
break;
case 561:
mydiv='Catherine_Palace',special='j6sxwx_VvNQ',destroyed=''
break;
case 562:
mydiv='Usayama_Castle',special='Y-V5bGb5TFc',destroyed='I_GBpKq2u5I'
break;
case 567:
mydiv='Tsuyama_Castle',special='',destroyed=''
break;
case 568:
mydiv='Loket_Castle',special='',destroyed=''
break;
case 569:
mydiv='Kagoshima_Castle',special='54-DM58UYIo',destroyed=''
break;
case 576:
mydiv='Peterhof_Palace',special='0T9K54T3S-g',destroyed='DJuG7brA6vs'
break;
case 577:
mydiv='Chigachishi_Castle',special='vlMMd-faQPs',destroyed=''
break;
case 582:
mydiv='Chateau_dAngers',special='lWf0BxdSD_M',destroyed='osjRG7Qrr5k'
break;
case 590:
mydiv='Nottingham_Castle',special='O7Kzuq1hvW4',destroyed=''
break;
case 593:
mydiv='Kii_Castle',special='YCw_henRtTk',destroyed=''
break;
case 606:
mydiv='LasVegas_Castle-Hotel',special='JYswuyVjtyg',destroyed=''
break;
case 607:
mydiv='Komaki_Yamashiro',special='dLLMnCROjaw',destroyed='xjTwTF1PmKQ'
break;
case 613:
mydiv='Vaijayanta',special='TAe0EyeoHnM',destroyed=''
break;
case 617:
mydiv='Uwajima_Castle',special='oD10vPr-Dg4',destroyed='670E26LpWhA'
break;
case 638:
mydiv='Chillingham_Castle',special='biWkdDUrUHU',destroyed=''
break;
case 650:
mydiv='Mt.Kongo_Castle',special='S_uLUkNeUoE',destroyed=''
break;
case 671:
mydiv='Inframuros',special='Xjt79zZqG8k',destroyed=''
break;
case 674:
mydiv='Matsukura_Castle',special='xsIDsvWJPEo',destroyed=''
break;
case 675:
mydiv='Aragonese_Castle',special='',destroyed='yt3xxG0q1yA'
break;
case 688:
mydiv='Hohenzollern_Castle',special='0KdT27Pl8qM',destroyed=''
break;
case 689:
mydiv='Buyo_Dance_Academy',special='m12vutdiG1A',destroyed=''
break;
case 997:
mydiv='Mitsunari_Ishida',special='hQMnerfeW5c',destroyed=''
break;
case 998:
mydiv='Stephan_the_Great',special='NgsVYm8QukY',destroyed=''
break;
case 999:
mydiv='Kuroda_Nagamasa',special='5-TpStMuHB4',destroyed='-cLc8-rO8sM'
break;
default:
mydiv=''
break;
}
switch (cod) {
case 1:
if (mydiv!=''&&special!=''&&destroyed!=''){
document.getElementById(numero).innerHTML="<font color=green onClick='oshiro("+numero+",2)'>["+mydiv+"]</font>"
}
if (mydiv!=''&&special!=''&&destroyed==''){
document.getElementById(numero).innerHTML="<font color=blue onClick='oshiro("+numero+",2)'>["+mydiv+"]</font>"
}
if (mydiv!=''&&special==''&&destroyed!=''){
document.getElementById(numero).innerHTML="<font color=yellow onClick='oshiro("+numero+",2)'>["+mydiv+"]</font>"
}
if (mydiv!=''&&special==''&&destroyed==''){
document.getElementById(numero).innerHTML=""
}
break;
case 2:
if (mydiv!=''&&special!=''&&destroyed!=''){
document.getElementById(numero).innerHTML="<br><br><b>"+mydiv+"</b><br><iframe src=https://www.youtube.com/embed/"+special+" width='640' height='360' frameborder='0'></iframe><iframe src=https://www.youtube.com/embed/"+destroyed+" width='640' height='360' frameborder='0'></iframe><br><br>"
}
if (mydiv!=''&&special!=''&&destroyed==''){
document.getElementById(numero).innerHTML="<br><br><b><font color=blue>"+mydiv+"</font></b><br><iframe src=https://www.youtube.com/embed/"+special+" width='640' height='360' frameborder='0'></iframe><br><br>"
}
if (mydiv!=''&&special==''&&destroyed!=''){
document.getElementById(numero).innerHTML="<br><br><b><font color=yellow>"+mydiv+"</font></b><br><iframe src=https://www.youtube.com/embed/"+destroyed+" width='640' height='360' frameborder='0'></iframe><br><br>"
}
break;
}
}

// LINK GDRV DROPBOX YOUTUBE (img  width='140')
// <br><a target='_blank' href=><img onClick='stage("+numero+",2)' src=https://i.imgur.com/pJFCm25.png></a>
// https://drive.google.com/uc?id="+gdrv+"
// {window.open('https://www.dropbox.com/sh/4yvtvzi0bgvm6ky/AAD9c--Ke68LHTn2E1Nd3QVOa?dl=0','_blank')}
//<iframe src=https://www.youtube.com/embed/"+youtube+"?autoplay=1 width='430' height='240' frameborder='0'></iframe>
